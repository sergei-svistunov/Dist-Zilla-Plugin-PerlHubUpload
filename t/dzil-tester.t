    ...
    {dist_root => 'dzil-tester/DZT'},
    {
        add_files => {
            'source/dist.ini' => dist_ini(
                ...
                'GatherDir',
                'MakeMaker',
                ['PerlHubUpload' => {debuild_args => '-S -sa -us -uc', dput_args => '--simulate --unchecked'}]
            )
        }
    }
);
...
ok(scalar(grep {/^\Q[PerlHubUpload]   Simulated upload.\E$/} @{$zilla->log_messages}), 'Checking uploaded status');
